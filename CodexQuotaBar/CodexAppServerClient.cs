...
public sealed class CodexAppServerClient : IAsyncDisposable
{
    ...
    private async Task InitializeAsync(CancellationToken cancellationToken)
    {
        ...
        using var _ = await RequestAsync(
            ...
            new
            {
                clientInfo = new { name = "codex_quota_bar", title = "Codex Quota Bar", version = "1.0.6" },
    ...
    private static void TryResetLog()
    {
        try
        {
            File.WriteAllText(DiagnosticLogPath, $"{DateTimeOffset.Now:O} Codex Quota Bar 1.0.6{Environment.NewLine}");
        }
        catch
        {
